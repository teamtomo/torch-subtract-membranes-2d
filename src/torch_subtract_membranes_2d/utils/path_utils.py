...
def rasterize_path(
    ...
    upper_bound = torch.tensor(image_shape) - 1
    ...
    rasterized_path.index_put_(indices=(idx_h, idx_w), values=torch.ones(len(points), dtype=dtype), accumulate=False)
...
def find_pixels_around_path(
    ...
    # rasterize path
    rasterized_path = rasterize_path(path=path, image_shape=image_shape)
...
def sample_image_along_path(
    ...
    upper_bound = torch.tensor([h - 1, w - 1])
...
def center_of_mass_1d(a: torch.Tensor) -> torch.Tensor:
    indices = torch.arange(len(a), device=a.device)
    weights_normalized = a.abs() / a.abs().sum()
    center_of_mass = torch.sum(indices * weights_normalized)
