...
def test_open_path_interpolation():
    ...
    assert torch.allclose(interp_u0, torch.tensor([0.0, ]), atol=1e-5)

    u0_5 = torch.tensor([0.5]).float()
    interp_u0_5 = path.interpolate(u0_5)
    assert torch.allclose(interp_u0_5, torch.tensor([1.5, ]), atol=1e-5)

    u1 = torch.tensor([1]).float()
    interp_u1 = path.interpolate(u1)
    assert torch.allclose(interp_u1, torch.tensor([3.0, ]), atol=1e-5)
...
def test_closed_path_interpolation():
    ...
    assert torch.allclose(interp_u0, torch.tensor([0.]), atol=1e-5)

    # should be 3 in middle...
    u0_5 = torch.tensor([0.5]).float()
    interp_u0_5 = path.interpolate(u0_5)
    assert torch.allclose(interp_u0_5, torch.tensor([3.0, ]), atol=1e-5)


def test_optimization():
    # setup some initial control points
    initial_control_points = torch.zeros(size=(30, ))
